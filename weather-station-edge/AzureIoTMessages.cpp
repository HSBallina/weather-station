...
#include "AzureIoTMessageProperties.h"
    ...
    size_t *payload_buffer_length)
{
    ...
    readings rOut = getReadings(outdoor);
    readings rIn = getReadings(indoor);
    ...
    rc = az_json_writer_append_double(&jw, rOut.temperature, DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(
    ...
    rc = az_json_writer_append_double(&jw, rOut.humidity, DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(
    ...
    rc = az_json_writer_append_double(&jw, rOut.pressure, DOUBLE_DECIMAL_PLACE_DIGITS);
    EXIT_IF_AZ_FAILED(
    ...
    size_t *response_length)
{
            ...
            &component_name)))
    {
                ...
                &jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_TELEMETRY_FREQ_SECS)))
        {
            ...
            azurePnpSetTelemetryFrequency((size_t)value);
